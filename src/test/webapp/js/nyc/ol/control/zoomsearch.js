...
QUnit.test('zoom', function(assert){
	...
	var control = new nyc.ol.control.ZoomSearch(this.TEST_MAP);
	var zoom = this.TEST_MAP.getView().getZoom();
	$('#btn-z-in').trigger('click');
	assert.equal(this.TEST_MAP.getView().getZoom(), zoom + 1);
	$('#btn-z-out').trigger('click');
	assert.equal(this.TEST_MAP.getView().getZoom(), zoom);
});
